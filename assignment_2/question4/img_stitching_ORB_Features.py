...
img_train = cv2.imread("capture_isp_1.png") #trainImg
img_query = cv2.imread("capture_isp_2.png") #queryImg
gray_img_train = cv2.cvtColor(img_train, cv2.COLOR_RGB2GRAY)
gray_img_query = cv2.cvtColor(img_query, cv2.COLOR_RGB2GRAY)

ks_train, features_train = keyPointsFeatures(gray_img_train) 
ks_query, features_query = keyPointsFeatures(gray_img_query) 

matches = matchKeyPoints(features_train, features_query, 0.75)
temp_img = cv2.drawMatches(img_train, ks_train, img_query, ks_query, np.random.choice(matches,100), None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
#print(ks_train)
#print(ks_query)
print(matches)
M = getHomography(ks_train, ks_query, matches, 4)
print(M)
...
plt.imshow(result)
cv2.imwrite("imageStichORB.png", result)
plt.show()
