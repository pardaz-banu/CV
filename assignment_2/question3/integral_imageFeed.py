...
def integrate(im, start, end):
    ...
    for i in range(bit_perm):  
        ...
        S += [sign * im[tuple(corner_points[r])] if(not bad[r]) else 0
              for r in range(rows)]
    return S



def integral_image(img, *, dtype=None):
    if dtype is None and img.real.dtype.kind == 'f':
        dtype = np.promote_types(img.dtype, np.float64)

    st = img
    for i in range(img.ndim):
        st = st.cumsum(axis=i, dtype=dtype)
    return st
